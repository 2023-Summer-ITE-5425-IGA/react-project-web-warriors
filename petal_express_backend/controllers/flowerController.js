...
const getFlowers = asyncHandler(async (req,res) =>{
    ...
    .sort({ f_id: 1 }).exec(); // Sort by f_id in ascending order
...
const updateFlowerById = asyncHandler(async (req,res) =>{
    ...
    const updatedFlower = await flowerModel.findByIdAndUpdate(objectId,req.body,{new:true});  //new:true will return the data after the update
